...
function CoordinatesPush(varargin)
    ...
    if exist(fullfile(pathName, fileName), 'file') == 2
        ...
        if loadOK
			...
			aaa = cell(length(handles.ImportFiles), 1)
...
function cellData = assignROIsToCellData(cellData, roiArray, nDataColumns)    
    ...
    for k = 1:length(cellData)
        ...
        cellData{k}(:, nDataColumns + 1) = 0;  % 将所有行的最后一列设为0
        
        for m = 1:length(roiArray{k})
            % Iterate over each ROI in that cell
			% inpolygon 判断点是否在多边形内部 inpolygon , return true/false
			% 5，6 两列，，对应数据集中的x, y坐标
            ...
            for chan = 1:handles.Nchannels
                ...
                for c = 1:size(handles.CellData, 1);  %第一维度，估计是统计数量
                    ...
                    for roiInc = 1:length(handles.ROICoordinates{c}); % 画框的区域
                        ...
                        % Since which ROI a point falls in is encoded in binary, decode here
                        whichPointsInROI = fliplr(dec2bin(handles.CellData{c}(:,handles.NDataColumns + 1))); % fliplr左右翻转二进制数组
                        ...
                        if ~isempty(dataCropped)
                            ...
                            [~, ClusterSmoothTable{roiInc, c}, ~, classOut, ~, ~, ~, Result{roiInc, c}] = DBSCANHandler( ...
                                    dataCropped(dataCropped(:,12) == chan, 5:6), ...
                                    dbscanParams, c, roiInc, true, true, clusterColor, ...
                                    dataCropped(dataCropped(:,12) == chan, handles.NDataColumns + 2) ...
                                );
...
%                             handles = AppendToROITable(handles, Result);
                ...
                %fullfile用于路径拼接 'ClusterSmoothTable','Result'是两个变量
...
% disp('clusterIDAssign');
    ...
    disp("length of CellData: ")
    disp(length(handles.CellData))
    for k = 1:length(handles.CellData)

        disp("length of ROICooridnates: ")
        disp(length(handles.ROICoordinates{k}))
        for m = 1:length(handles.ROICoordinates{k})
            % Since which ROI a point falls in is encoded in binary, decode here
